...
void 
fill_down (Triangle tri,vtkImageData *image, int width, int height, std::vector<double> label_xy) {
    //cout <<"\n"<<endl;
...
void 
fill_up (Triangle tri,vtkImageData *image, int width, int height, std::vector<double> label_xy) {
    //cout <<"\n"<<endl;
    ...
    // 0-2: x; 3-5:y; 6: rowMin, 7:rowMax;
        ...
        for (int y=ceil_441(rowMin); y<=floor_441(rowMax);y++) { //change for fill_up
            ...
            if ( floor_441(rightEnd) - ceil_441(leftEnd) <0) {  
   ...
   // tri_size
    for (int t=0;t<tri_size;t++) { 
        ...
        if (ly==my) {
            ...
            if (ly==rowMin) {
                fill_up (triangles[t],image,screen.width,screen.height,label_xy);
            }
            
            
            if (ly==rowMax) {
                fill_down (triangles[t],image,screen.width,screen.height,label_xy);
            }
        ...
        if (my==ry) {
            ...
            fill_up (UDTriangle[0],image,screen.width,screen.height,label_up);
            std::vector<double> label_down = basic_setup(UDTriangle[1]);
            fill_down (UDTriangle[1],image,screen.width,screen.height,label_down);
        }
