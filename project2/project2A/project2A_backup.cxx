...
// guess, set vector<Triangle> triangles as a data member as the vtk441Mapper class
class vtk441MapperPart1 : public vtk441Mapper
{
 public:
   static vtk441MapperPart1 *New();
   
   // inside this renderPiece, render colors glColor3ub
   virtual void RenderPiece(vtkRenderer *ren, vtkActor *act)
   {
      cerr<<"enter Mapper1 RenderPiece"<<endl;
      RemoveVTKOpenGLStateSideEffects();
      SetupLight();
      ...
      for (int t=0;t<triangles.size();t++) {
        ...
        for (int v=0;v<3;v++) {
          ...
          glColor3ub(ColorMap[color_index*3],ColorMap[color_index*3+1],ColorMap[color_index*3+2]);
...
class vtk441MapperPart2 : public vtk441Mapper
{
 public:
   ...
   // inside RenderPiece, where to render the textures
   /*
    slide 18
    take color map, make it into texture,
    move the texture to the GPU.
    Inside the triangle loop, call glTexCoordlf 
   */
   virtual void RenderPiece(vtkRenderer *ren, vtkActor *act)
   {
       RemoveVTKOpenGLStateSideEffects();
...
/*
Q: 1, What's difference between color and texture?
2, Which is correct answer we should use "diff" to compare with?

GetColorMap: return *ptr = new unsigned char[256*3]
Triangle.fieldValue: between 0 to 1

For part 1, color
use fieldValue
colorMap[floor(fieldValue * 255)];



3, How to draw 12 lines frame?
