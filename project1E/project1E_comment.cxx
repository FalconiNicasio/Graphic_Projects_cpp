...
Camera::CameraTransform() {
    ...
    //cout << "Camera Frame: U = "<<std::setprecision(6)<<u[0]<<", "<<u[1]<<", "<<u[2]<<endl;
...
RenderTriangles(Matrix TransformMatrix,vector<Triangle> triangles, vtkImageData *image,Screen screen) {
    ...
    for (int t=0;t<triangles.size();t++) {
        ...
        for (int j=0;j<3;j++) {
            //cout << "Transformed V"<<j<<" from ("<<triIn.X[j]<<", "<<triIn.Y[j]<<", "<<triIn.Z[j]<<") to ("<<triOut.X[j]<<", "<<triOut.Y[j]<<", "<<triOut.Z[j]<<")"<<endl;
        }   
        // draw triangle
        draw_triangle(image,screen,triOut);
        //function return image pointer that passed in, wait, no, function does not need to return since the image alredy changed.
    }  
}
/* buffer and image, only need to pass one.
Why: Because buffer is used to access the image's pixel in the specific location. 
If pass image, create buffer in the function to access the specific location
If pass buffer, it is the pointer that directly points to the image. Use index to reach the specific pixel.
*/
...
int main()
{
    ...
    // TODO: Change this i and t in RenderTriangles(line 776)
    for (int i=0;i<4;i++) {
        //cout <<"Frame "<<i*250<<endl;
        vtkImageData *image = NewImage(1000,1000);
        Screen screen = InitializeScreen(image,1000,1000); //InitializeScreen(image,width,height)
        // Q: What's function of screen.buffer? Because the pixels are directly write to image, not use screen.buffer any other places except initialize
        // the way we initialize it make the screen has the same pixel as the image, but what's the function of it?
        // In general, what's the function of the Screen class except z-buffer?
